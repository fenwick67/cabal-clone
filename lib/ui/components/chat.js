...
module.exports = {
  ...
  computed:{
    ...
    sortedUsers:function(){
      return _.sortBy(this.users,u=>u.online?0:1);
    }
  },
  template:`
  <div class="chat">
      ...
      <div class="chat-header-inner">
        ...
        <poppins>
          <button slot="button" class="button button__white button__small" title="peer connections">
            <span v-if="users.filter(p=>p.online).length > 0" class="online-circle"></span>
            <span v-else class="offline-circle"></span>
           {{ users.filter(p=>p.online).length }}/{{ users.length }}
          </button>
          <div slot="tooltip">
            <ul class="user-list">
      ...
      <li class="chat-entry" v-for="chain in messageChains">
        ...
        <span class="chat-entry-content">
          <span class="chat-entry-info">
            <username :cabal="cabal" :userid="chain[0].key"/>
